 ...
 */
int my_comparer(const void *this, const void *that)//cmpare two processes, sort by arrival and priority
{
  Process *this_p = (Process*)this; 
  Process *that_p = (Process*)that;
    
  if (this_p->priority < that_p->priority){
    return 1;
   }
  else if(this_p->priority > that_p->priority){
    return -1;
  }
  else{
    if(this_p->arrival_time < that_p->arrival_time){
      return -1;
    }
    else if(this_p->arrival_time > that_p->arrival_time){
      return 1;
    }
    else{
      return 0;
